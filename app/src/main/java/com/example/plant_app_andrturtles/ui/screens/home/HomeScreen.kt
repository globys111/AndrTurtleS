package com.example.plant_app_andrturtles.ui.screens.home
...
import com.example.plant_app_andrturtles.ui.viewmodel.HomeViewModel
        ...
        when (val s = state) {
            ...
            is UiState.Success -> {
                ...
                val tasksForSelected = remember(data.allOccurrences, data.selectedDate) {
                    TaskEngine.tasksForDate(data.allOccurrences, data.selectedDate)
                }

                val tasksToday = remember(data.allOccurrences, data.today) {
                    TaskEngine.tasksForDate(data.allOccurrences, data.today)
                }
                    ...
                    item {
                        PlantsSection(
                            ...
                            topTasksForPlant = { plantId ->
                                TaskEngine.top3ForPlant(data.allOccurrences, plantId)
                            },
